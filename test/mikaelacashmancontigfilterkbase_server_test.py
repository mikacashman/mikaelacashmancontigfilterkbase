...
class mikaelacashmancontigfilterkbaseTest(unittest.TestCase):

    @classmethod
    def setUpClass(cls):
        token = environ.get('KB_AUTH_TOKEN', None)
        cls.ctx = {'token': token, 'provenance': [{'service': 'mikaelacashmancontigfilterkbase',
        ...
        for nameval in config.items('mikaelacashmancontigfilterkbase'):
            cls.cfg[nameval[0]] = nameval[1]
        cls.wsURL = cls.cfg['workspace-url']
        cls.wsClient = workspaceService(cls.wsURL, token=token)
        cls.serviceImpl = mikaelacashmancontigfilterkbase(cls.cfg)
    ...
    def getWsName(self):
        ...
        wsName = "test_mikaelacashmancontigfilterkbase_" + str(suffix)
